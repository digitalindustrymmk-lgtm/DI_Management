...
const EmployeeRow = memo(({ employee, onEdit, onDelete, onInlineUpdate, index, settings, isSelected, onToggleSelect, onNavigate }) => {

    // Helper to wire up navigation easily for every cell
    // It creates the unique ID and the event handler
    const navProps = (colKey) => ({
        id: `cell-${index}-${colKey}`,
        onKeyDown: (e) => {
            if (onNavigate) {
                onNavigate(e, index, colKey);
            }
        }
    });
    ...
    return (
        <tr className={`group transition-colors animate-fade-in border-b border-slate-100 last:border-0 ${isSelected ? 'bg-indigo-50' : 'hover:bg-indigo-50/30'}`} onDoubleClick={onEdit}>
                ...
                <div className="flex items-center">
                    ...
                    <div className="ml-3">
                        <div className="font-bold text-slate-800 text-sm">
                            <EditableCell 
                                value={employee.name} 
                                onSave={(val) => onInlineUpdate(employee.id, 'ឈ្មោះ', val)} 
                                {...navProps('name')}
                            />
                        </div>
                    </div>
            ...
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600 font-medium">
                <EditableCell 
                    value={employee.latinName} 
                    onSave={(val) => onInlineUpdate(employee.id, 'ឈ្មោះឡាតាំង', val)} 
                    {...navProps('latinName')}
                />
            </td>
            ...
            <td className="px-4 py-3 whitespace-nowrap text-sm">
                <EditableCell 
                    ...
                    onSave={(val) => onInlineUpdate(employee.id, 'ភេទ', val)} 
                    options={GENDER_OPTIONS} 
                    className={`font-semibold ${employee.gender === 'ស្រី' ? 'text-pink-500' : 'text-indigo-500'}`}
                    {...navProps('gender')}
                />
            </td>
            ...
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600 font-mono">
                <div className="bg-slate-100 px-2 py-1 rounded text-center text-xs font-bold text-slate-700">
                    <EditableCell 
                        value={employee.studentId} 
                        onSave={(val) => onInlineUpdate(employee.id, 'អត្តលេខ', val)} 
                        {...navProps('studentId')}
                    />
                </div>
            </td>
            ...
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600">
                <EditableCell 
                    ...
                    onSave={(val) => onInlineUpdate(employee.id, 'ជំនាញ', val)} 
            ...
            {/* 6. GROUP */}
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600 font-semibold text-slate-700">
                <EditableCell 
                    value={employee.group} 
                    onSave={(val) => onInlineUpdate(employee.id, 'ក្រុម', val)} 
            ...
            {/* 7. CLASS */}
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600 text-[10px] text-slate-400">
                <EditableCell 
                    value={employee.class} 
                    onSave={(val) => onInlineUpdate(employee.id, 'ថ្នាក់', val)} 
            ...
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600">
                <EditableCell 
                    ...
                    onSave={(val) => onInlineUpdate(employee.id, 'ផ្នែកការងារ', val)} 
            ...
            {/* 9. POSITION */}
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600">
                <div className="px-2 py-1 bg-indigo-50 text-indigo-600 rounded border border-indigo-100 text-xs font-medium text-center">
                    <EditableCell 
                        value={employee.position} 
                        onSave={(val) => onInlineUpdate(employee.id, 'តួនាទី', val)} 
            ...
            {/* 10. ACADEMIC YEAR (Composite Cell) */}
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600">
                <div className="flex flex-col">
                    <span>
                        <EditableCell 
                            value={employee.academicYear} 
                            onSave={(val) => onInlineUpdate(employee.id, 'ឆ្នាំសិក្សា', val)} 
                            options={ACADEMIC_YEARS} 
                            {...navProps('academicYear')}
                        />
                    </span>
                    <span className="text-[10px] text-slate-400 flex items-center gap-1">
                        Gen: 
                        <EditableCell 
                            value={employee.generation} 
                            onSave={(val) => onInlineUpdate(employee.id, 'ជំនាន់', val)} 
                            options={GENERATIONS} 
                            {...navProps('generation')}
                        />
                    </span>
            ...
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600">
                <EditableCell 
                    value={employee.dob} 
                    onSave={(val) => onInlineUpdate(employee.id, 'ថ្ងៃខែឆ្នាំកំណើត', val)} 
                    {...navProps('dob')}
                />
            </td>

            {/* 12. POB */}
            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600 max-w-xs truncate" title={employee.pob}>
                <EditableCell 
                    value={employee.pob} 
                    onSave={(val) => onInlineUpdate(employee.id, 'ទីកន្លែងកំណើត', val)} 
                    {...navProps('pob')}
                />
            </td>

            {/* 13. TELEGRAM */}
            <td className="px-4 py-3 whitespace-nowrap text-sm text-blue-500 cursor-pointer font-medium hover:underline">
                <EditableCell 
                    value={employee.telegram} 
                    onSave={(val) => onInlineUpdate(employee.id, 'តេឡេក្រាម', val)} 
                    {...navProps('telegram')}
                />
            </td>

            {/* 14. SCHEDULE DAYS (Mon-Sun) */}
            {['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'].map((day, idx) => {
                const dayKh = ['ចន្ទ', 'អង្គារ៍', 'ពុធ', 'ព្រហស្បត្តិ៍', 'សុក្រ', 'សៅរ៍', 'អាទិត្យ'][idx];
                return (
                    <td key={day} className="px-4 py-3 whitespace-nowrap text-center border-l border-slate-100 text-sm font-medium text-slate-700 bg-slate-50/30">
                        <EditableCell 
                            value={employee[day]} 
                            onSave={(val) => onInlineUpdate(employee.id, `កាលវិភាគ/${dayKh}`, val)} 
                            options={settings?.schedules} 
                            // Matches 'schedule-mon', 'schedule-tue' etc. from Parent List logic
                            {...navProps(`schedule-${day}`)}
                        />
                    </td>
                );
            })}
